...
export default {
  data: () => ({
    ...
    rules: {
      image: [(value) => !!value || "Bank Code is required"],
      status: [(value) => !!value || "Bank Name is required"],
    },
  }),
  ...
  methods: {
    ...
    editItem() {
      ...
      this.models = { image: data.image, status: data.status };
      this.dialog = true;
    },

    submit() {
      let dataUpdate = this.selectedItem;
      let image = this.models.image
